...
if (!function_exists('sng_get_entry_link_data')) {
  function sng_get_entry_link_data($id, $thumb_size, $is_date, $show_alternate_title = false) {
    ...
    $size = $thumb_size === 'thumb-160' ? [160, 160] : [520, 300];
...
if (!function_exists('sng_entry_link')) {
  function sng_entry_link($atts)
  {
    ...
    foreach ($ids as $eachid) {
      ...
      if ($url && $title) {
        $output .= <<<EOF
<a class="linkto table" href="{$url}"{$target}><span class="tbcell tbimg">{$img}</span><span class="tbcell tbtext">{$date}{$title}</span></a>
EOF;
      }
    } // endforeach
    return $output;
    ...
    switch ($type) {
      ...
      case "card2":
        ...
        foreach ($array_posts as $i => $post) {
          ...
          $output .= sng_longcard_link(array('id' => $post->ID, 'is_alternate_title' => $showAltenateTitle));
        }
        break;
      case "card3":
        $shown_count = 0;
        ...
        foreach ($array_posts as $i => $post) {
          ...
          $output .= sng_longcard_link_half(array('id' => $post->ID, 'is_alternate_title' => $showAltenateTitle));
        }
        $output = '<div class="'. $class_name. '">'.$output.'</div>';
        break;
      // sng_sidelong_card
      default:
        foreach ($array_posts as $post) {
          $output .= sng_entry_link(array('id' => $post->ID, 'is_date' => $is_date, 'is_alternate_title' => $showAltenateTitle));
        }
    }
    return $output;
  }
 ...
 * 線・点線を出力
 *********************/
function sng_sen($atts) { return '<hr>'; }
function sng_tensen($atts){ return '<hr class="dotted">'; }
